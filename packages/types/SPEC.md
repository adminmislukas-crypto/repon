...
| `src/ofertas.ts` | `OfferKind`, `OfferStatus`, `OfferItem` (+ variantes `OfferItemReactiva`/`OfferItemProactiva`/`OfferItemAlt`), `Offer`, `NuevoOfferItem` (+ variantes `NuevoOfferItemReactiva`/`NuevoOfferItemProactiva`, tipo nombrado propio, no un alias de `OfferItem`), `DatosEntrega`, `SolicitudElegible` (+ `SolicitudElegibleItem`; delta `backend-core-api-ofertas`, D14) |
