...
// (`PorcentajeInvalidoError`, Phase 6; `ArchivoCargaInvalidoError`, Phase
// 5a; `CatalogQueryUnavailableError`, cross-domain only) is a one-line
// append — mirrors `IdentidadExceptionFilter` exactly.
 ...
 *
 * `@Catch()` deliberately lists only these 3 classes (this PR's scope):
 * anything else (a guard rejection, an unexpected infra error) does not
 * match, so Nest falls through to `main.ts`'s `GlobalExceptionFilter`
 * instead of this filter inventing a second, competing catch-all.
 */
@Catch(CatalogItemNotFoundError, EmpresaNoActivaError, PrecioInvalidoError)
