...
## PR9 (next batch) starting point

Only Phase 9 (closure) remains — 6 SPEC.md deltas (2 originally declared in the proposal + 4 found during design/apply: `companyStatus` gate, visibility-filter-scope correction, `CatalogQueryPort`/`CatalogRepository` shape, `CatalogProductRepository` gap-fill from PR3b, the 413-vs-400 split from the orchestrator's PR5b fix-forward, and `identidad/SPEC.md`'s `reactivarEmpresa`/`EmpresaReactivada` addition) + module-exports audit + full-suite verification. No new production code expected. After PR9, this change is ready for `sdd-verify`.
