 ...
 * initial `loadSession()` effect is still in flight races it (two
 * overlapping `act()` scopes) in a way the real app structurally cannot.
 */
...
async function submit(
  ...
  // `fireEvent.*` returns a Promise in this RNTL version — each call must be
  // awaited before the next, or the controlled `TextInput`s' state updates
  // haven't committed yet when `press` fires (the submit button would still
  // see `email`/`password` as empty and stay `disabled`).
...
describe('LoginScreen', () => {
  ...
  it('a server-side role mismatch (403 ROL_NO_PERMITIDO) shows the mapped message and writes nothing to storage', async () => {
    ...
    expect(screen.getByTestId('login-error')).toHaveTextContent(
      'Esta cuenta no corresponde a un usuario — usa la app de proveedores.',
    );
