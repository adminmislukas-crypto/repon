...
## Fix-forward — lote `09` (`grants_identidad_service_role`)

Migrado en `supabase/migrations/20260804090000_09_grants_identidad_service_role.sql`. No agrega columnas ni tablas — corrige un bug de permisos descubierto durante `backend-core-api-foundation` PR 4, en las tablas ya migradas por los lotes `01a`/`01b`.

**El bug**: `companies`, `company_dispatch_zones`, `profiles` y `admin_roles` nunca tuvieron ningún grant explícito para `service_role`. El comentario de `01b` (ver sección 6 de `20260803120110_01b_identidad_admin.sql`) afirmaba que esto no importaba porque `service_role` "hace bypass de RLS (BYPASSRLS) y no necesita ningún grant explícito" — **esa afirmación es incorrecta**: `BYPASSRLS` solo omite las *políticas* de row-level security, no tiene ningún efecto sobre los privilegios de objeto estándar (`GRANT`/`REVOKE` a nivel de tabla), que son un mecanismo de Postgres completamente distinto. Verificado empíricamente contra una instancia real de Postgres local: conectar como `service_role` (la conexión de `core-api` corre `SET ROLE service_role` después de conectar como `authenticator`, igual que PostgREST) e intentar `SELECT`/`UPDATE` contra `companies` devolvía `permission denied for table companies`.

**El fix**: `grant select, insert, update on <tabla> to service_role` para las cuatro tablas — mismo patrón que `20260803120700_07_auditoria.sql` ya usaba correctamente para `audit_log`. Sin `DELETE` en ninguna: mismo principio de "no hay baja física, solo `UPDATE` de `status`" que el resto del schema. `anon`/`authenticated` no cambian — sus grants (o su ausencia total, en el caso de `admin_roles`) siguen siendo los que definieron `01a`/`01b`.

Rollback: `supabase/rollback/09_grants_identidad_service_role_down.sql`. Test: `supabase/tests/09_grants_identidad_service_role_test.sql` (DML directo con `set local role service_role`, no `table_privs_are` — mismo motivo que la nota de `v_auth_orphans` en "Columnas reales — lote `01b`": el stack local de Supabase ya otorga privilegios amplios por defecto a `service_role` sobre `public`, independientes de este lote).

## Fix-forward — lote `10` (`grants_domain_tables_service_role`)

Migrado en `supabase/migrations/20260804090500_10_grants_domain_tables_service_role.sql`. Mismo bug que el lote `09` (ver esa sección arriba para la explicación completa de por qué `BYPASSRLS` no sustituye un `GRANT`), alcance más amplio: una auditoría posterior contra una instancia real de Postgres local (misma consulta `information_schema.role_table_grants` que descubrió el bug de `01a`/`01b`) encontró el mismo root cause en **todas** las migraciones de dominio restantes — `02_consumo.sql` a `06_pedidos_pagos.sql` solo escribían `grant select ... to authenticated` (lectura directa de cliente) y `revoke all ... from anon, authenticated`, nunca un grant explícito a `service_role`. Solo `20260803120700_07_auditoria.sql` lo tenía bien desde el inicio.

**El fix**: `grant select, insert, update on <tabla> to service_role` para 11 tablas — `catalog_products`, `consumption_logs`, `offer_items`, `offers`, `orders`, `payments`, `pets`, `provider_catalog`, `refill_items`, `refill_requests`, `user_consumption`. Sin `DELETE` en ninguna, mismo principio de siempre.

**Excepción — `order_items`**: solo `grant select, insert` (**sin** `update`). `06_pedidos_pagos.sql` ya corre `revoke update, delete on public.order_items from anon, authenticated, service_role` para sostener la garantía de snapshot inmutable (D-6) — otorgar `update` aquí habría deshecho en silencio esa garantía. `service_role` sigue sin poder hacer `UPDATE` sobre `order_items`, igual que antes de este lote.

Rollback: `supabase/rollback/10_grants_domain_tables_service_role_down.sql`. Test: `supabase/tests/10_grants_domain_tables_service_role_test.sql` (DML directo con `set local role service_role`, mismo patrón que el lote `09`, encadenando fixtures a través de los 5 dominios; incluye el caso negativo explícito de `order_items` UPDATE — `throws_ok` con `42501` — para blindar la garantía D-6 contra una regresión futura).
...
| Tabla | Regla |
|---|---|
| `companies` | (lote `01a`, migrado) `authenticated` ve `status = 'activo'` (directorio público), más su propia empresa en cualquier status vía `EXISTS` sobre `profiles.company_id`. `service_role` tiene `select, insert, update` explícitos desde el lote `09` (fix-forward, ver "Fix-forward — lote `09`" más abajo) — antes de ese lote no tenía ningún grant, pese a que `01a`'s prosa original ya asumía que `core-api` podía leer/escribir vía service-role |
| `company_dispatch_zones` | (lote `01a`, migrado) Hereda la visibilidad de su `companies` padre — mismo predicado activo-o-dueño, evaluado contra la fila padre. `service_role` tiene `select, insert, update` desde el lote `09` (mismo fix que `companies`, mismo root cause) |
| `profiles` | (lote `01a`, migrado) **Conflicto con la prosa original de esta fila, no sobrescrita en silencio**: la regla real implementada es solo lectura de la propia fila (`id = auth.uid()`), **sin** política UPDATE. "...y edita..." queda superseded por design.md D-1 — mutaciones como `suspenderUsuario` van por core-api con service-role, nunca UPDATE directo del cliente. Ver `db-schema-identidad` Requirement "profiles has no client-side mutation policy". **`service_role` tiene `select, insert, update` explícitos solo desde el lote `09`** (fix-forward) — hasta entonces ese "van por core-api con service-role" no funcionaba: `service_role` no tenía ningún grant sobre `profiles`, pese a `BYPASSRLS` |
| `pets`, `user_consumption` | (lote `02`, migrado) `authenticated` ve solo filas propias, `user_id = auth.uid()` — ni siquiera proveedores tienen acceso. `service_role` tiene `select, insert, update` explícitos solo desde el lote `10` (fix-forward, mismo bug que `companies`/`profiles` en el lote `09`) — antes de eso `service_role` no tenía ningún grant sobre ninguna de las dos |
| `consumption_logs` | (lote `02`, migrado) Sin columna de dueño propia (Q8) — SELECT vía `EXISTS` contra `user_consumption.user_id`, mismo patrón único que `refill_items`/`offer_items`/`order_items` (`db-access-control`). `service_role` tiene `select, insert, update` desde el lote `10` (mismo fix-forward que el resto de esta fila) |
| `refill_requests`, `refill_items` | (lote `04`, migrado) **Conflicto con la prosa original de esta fila, no sobrescrita en silencio**: la regla real implementada es owner-only, `user_id = auth.uid()` en `refill_requests` y `EXISTS` contra `refill_requests.user_id` en `refill_items` — **ningún** proveedor puede leerlas directamente. "...visibles (solo lectura) para proveedores cuyo catálogo coincide..." queda superseded por Q2/D1: el matching cross-tenant es RLS-inexpresable sin un predicado auto-derrotante, va por core-api (`buscarProveedoresCompatibles`, service-role) y la futura Edge Function `match-refill-request` (fuera de alcance de este cambio). Ver "Columnas reales — lote `04`" más abajo y `db-schema-refill-matching` Requirement "refill_requests SELECT is owner-only". **`service_role` tiene `select, insert, update` explícitos solo desde el lote `10`** — hasta entonces `buscarProveedoresCompatibles` no podía funcionar con esas credenciales pese a `BYPASSRLS` |
| `catalog_products` | (lote `03`, migrado) `authenticated` ve todas las filas sin filtro de status (Q6) — **sin** política ni grant `anon`, para no exponer el catálogo de referencia a scrapers sin sesión. `service_role` tiene `select, insert, update` desde el lote `10` (fix-forward) |
| `provider_catalog` | (lote `03`, migrado) **Conflicto con la prosa original de esta fila, no sobrescrita en silencio**: la regla real implementada es solo lectura, dos políticas `authenticated` — pública (`disponible = true`) y dueño (`EXISTS` sobre `profiles.company_id`, ve todo incl. `disponible = false`). No hay política de escritura para el cliente; `cargarProductoCatalogo` va por core-api con service-role, igual que `companies`/`profiles`. **`service_role` tiene `select, insert, update` explícitos solo desde el lote `10`** — hasta entonces `cargarProductoCatalogo` no podía escribir pese a `BYPASSRLS` |
| `offers`, `offer_items` | (lote `05`, migrado) **Conflicto con la prosa original de esta fila, no sobrescrita en silencio**: no hay política de escritura para el cliente — `enviarOferta`/`enviarOfertaProactiva`/`aceptarOferta` van por core-api con service-role, igual que el resto del cambio. `offers` tiene dos políticas SELECT `authenticated`: destinatario (`user_id = auth.uid()`, comparación directa — no `EXISTS` contra `refill_requests`, es lo que hace visibles a las ofertas proactivas sin solicitud) y proveedor (`EXISTS` sobre `profiles.company_id`, mismo patrón literal que `companies`/`provider_catalog`). `offer_items` usa el patrón `EXISTS`-contra-el-padre uniforme (Q8) sobre `offers.user_id` únicamente — el proveedor no tiene lectura directa de `offer_items` en este lote. Ver "Columnas reales — lote `05`" más abajo y `db-schema-ofertas` Requirement "SELECT allowlist for offers is required for Realtime to deliver anything". **`service_role` tiene `select, insert, update` explícitos en ambas tablas solo desde el lote `10`** — hasta entonces las mutaciones de core-api sobre `offers`/`offer_items` no podían ejecutarse pese a `BYPASSRLS` |
| `orders`, `order_items` | (lote `06`, migrado) `orders` tiene dos políticas SELECT `authenticated`: dueño (`user_id = auth.uid()`) y empresa (`EXISTS` sobre `profiles.company_id`, mismo patrón literal que `companies`/`offers`). `order_items` usa el patrón `EXISTS`-contra-el-padre (Q8) sobre `orders.user_id` únicamente — el proveedor no lee `order_items` directo, mismo carve-out que `offer_items` en lote `05`. `order_items` además es inmutable por grants (`revoke update, delete` incl. `service_role`, D-6). **`service_role` tiene `select, insert, update` explícitos en `orders` solo desde el lote `10`**; en `order_items` el lote `10` otorga únicamente `select, insert` — **nunca `update`**, para no deshacer la inmutabilidad D-6 que `06_pedidos_pagos.sql` ya fijaba vía `revoke` |
| `payments` | (lote `06`, migrado) **Conflicto con la prosa original de esta fila, no sobrescrita en silencio**: la regla real implementada es **cero acceso de cliente**, ni owner ni company — sin grant `select`, sin política. El estado del pago se lee vía core-api. Ver "Columnas reales — lote `06`" más abajo y `db-schema-pedidos-pagos` Requirement "payments has no direct client SELECT policy". **`service_role` tiene `select, insert, update` explícitos solo desde el lote `10`** — hasta entonces ese "se lee vía core-api" no funcionaba: `service_role` no tenía ningún grant sobre `payments`, pese a `BYPASSRLS` |
| `admin_roles` | (lote `01b`, migrado) Cero acceso de cliente — RLS habilitado + `revoke all` + cero políticas para `anon`/`authenticated` (los dos mecanismos de design.md D-2). Alcanzable vía `service role key` desde `admin-web`/core-api, pero **solo desde el lote `09`**: `01b`'s comentario original afirmaba que `service_role` no necesitaba ningún grant explícito porque "hace bypass de RLS (BYPASSRLS)" — incorrecto, `BYPASSRLS` no sustituye un `GRANT`; hasta el lote `09` no existía ningún privilegio real y `service_role` recibía `permission denied`. Ver "Fix-forward — lote `09`" más abajo. Ver "Bootstrap de administrador" más arriba para cómo se crea la primera fila |
