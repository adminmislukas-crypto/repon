...
**Delta `backend-core-api-consumo` (D1/D14)**: `adapters/scheduling/` se suma a la lista de subcarpetas de `adapters/`, también condicional — obligatoria únicamente si el dominio registra al menos un job programado (`@Cron()` de `@nestjs/schedule`); se omite si no. `consumo` es el primer y hoy único dominio con esta carpeta (`adapters/scheduling/consumption-check.job.ts`), cerrando formalmente el mismo gap que `core-api-hexagonal-layout` spec's WARNING-2 nombraba (delta ya declarada en `openspec/changes/backend-core-api-consumo/specs/core-api-hexagonal-layout/spec.md`, pendiente de fusión a `openspec/specs/` en el archive de este cambio).
