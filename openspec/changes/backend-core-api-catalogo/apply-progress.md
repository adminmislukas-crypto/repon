...
## PR8b — Cross-domain contract test

**Status**: done. **Implemented directly by the orchestrator**, not a sub-agent — the `sdd-apply` sub-agent hit the session's monthly spend limit again immediately on launch for this batch (no partial work product this time, working tree was clean). Given the pattern (2nd spend-limit hit this session, after PR3b), the orchestrator implemented this batch directly with its own tools rather than relaunching another large sub-agent call.

**Decision made explicitly (per PR8a's own handoff note, not silently picked)**: chose **option (b)** — real `EventEmitterPublisher`/`EventEmitter2` (`emitAsync`) + real `CompanyVisibilityListener` + real `OcultarCatalogoEmpresaUseCase`/`RestaurarCatalogoEmpresaUseCase`, with a MOCKED `CATALOG_VISIBILITY_PROJECTION` asserted via `toHaveBeenCalledWith`. This fully covers the actual named risk (a renamed event `type` string breaking the listener silently) without requiring a live Postgres in default CI, consistent with how every other e2e spec in this domain already mocks the repository/projection layer while keeping guard/pipe/filter/event-bus wiring real.

**Filename correction (caught before writing any test, not after)**: `tasks.md`/`design.md`'s prose names this file `catalogo-visibility.contract-spec.ts`. Verified against this repo's actual jest configuration before creating anything: `package.json`'s `jest` block has `rootDir: "src"` (the default `pnpm test:unit` run never scans `test/` at all), `test/jest-e2e.json` has `testRegex: ".e2e-spec\\.ts$"`, `test/jest-integration.json` has `testRegex: "\\.integration-spec\\.ts$"`. A file named `*.contract-spec.ts` matches **none** of these 3 patterns and would be silently skipped by every `pnpm test*` script — for a test whose entire purpose is being a "mandatory mitigation" that fails loudly in CI, that would be a correctness bug worse than not writing the test at all. Created as `services/core-api/test/catalogo-visibility.e2e-spec.ts` instead, with the reasoning documented in the file's own header comment.

**Real bug caught while writing this test (RED, not assumed GREEN)**: first draft called `moduleRef.get(CompanyVisibilityListener)` after `Test.createTestingModule({...}).compile()` and ran all 3 routing tests — 3 of 4 failed with "0 calls." Root cause: `@nestjs/event-emitter`'s `OnEvent` decorator is registered by an internal `EventEmitterReadinessWatcher` that hooks `onApplicationBootstrap`, and `.compile()` alone does **not** fire Nest lifecycle hooks — only `.init()` (or booting a full application) does. Fixed by calling `await moduleRef.init()` before resolving `EVENT_PUBLISHER`. Documented inline in the test file so a future reader doesn't hit the same gotcha. This is exactly the kind of thing an actually-executed contract test catches that a hand-written-but-never-run one would not.

**4 tests, all passing**: (1) real `EmpresaSuspendida` → `ocultarEmpresa` called with `(companyId, motivo)`, `mostrarEmpresa` not called; (2) real `EmpresaReactivada` → `mostrarEmpresa` called with `(companyId)`, `ocultarEmpresa` not called; (3) real `EmpresaAprobada` (constructor takes only `companyId`, verified) → routes to the SAME handler as reactivation, per design.md Diagram 2; (4) a rejecting mocked `ocultarEmpresa` does not propagate back through `eventPublisher.publish()` — proves the listener's fail-open/never-re-throw behavior (R4) with a real `emitAsync` round trip, not just a unit-mocked one.

**Gates**: `pnpm lint`/`typecheck`/`build`/`format:check` clean. `pnpm test`: 235 unit (unchanged from PR8a — this batch added zero unit tests, all 4 new tests are e2e-classified per the filename fix above) / 54 e2e (was 50, +4). Full suite includes `identidad`'s 120 unit + 22 e2e, byte-identical to PR7/PR8a's baseline — untouched by this batch.

**Commit**: `test(core-api): add catalogo cross-domain event contract test`.

**Risks**: none blocking. The mocked-projection choice (vs. a live-DB integration test) means this test does not prove `catalog_hidden_companies`'s actual SQL (upsert/update) is correct — that property is already proven separately by PR8a's own `kysely-catalog-visibility.projection.spec.ts` unit tests. This test's job is narrowly the STRING-KEYED WIRING risk, and it does that job.

## PR9 — Closure

**Status**: done. **Implemented directly by the orchestrator**, same as PR8b — no sub-agent launched for this batch given the session's 2 prior spend-limit hits; PR9 is docs-only (no TDD complexity, mostly editing 3 existing product SPEC.md files), well within direct-tool scope.

**Tasks 9.1–9.6, all complete**:
- `services/core-api/domains/catalogo/SPEC.md`: all 4 mutating signatures updated (`companyId`+`companyStatus` on all 4; `actualizarPrecio` additionally gains `companyId` where it had none before); `CatalogRepository` extended (`findById`/`saveMany`/`findByCompanyAndCategoria`); 2 new ports documented (`CatalogProductRepository`, `CatalogVisibilityProjection`); `CatalogQueryPort`'s failure semantics (`CatalogQueryUnavailableError`, never a degraded `[]`) documented; `PreciosCategoriaAjustados` added to "Eventos que publica"; "Eventos que consume" corrected to scope the visibility filter to cross-tenant reads only (not `buscarProductos`/`findByCompany`) and gained `EmpresaReactivada`.
- `services/core-api/domains/identidad/SPEC.md`: `reactivarEmpresa` added to `IdentidadInboundPort`, with the `suspenderEmpresa`-asymmetry rationale (fail-safe destination vs. permissive destination) written inline as a "Delta D16" note, matching this file's existing "Delta 5.4" convention. `EmpresaReactivada` added to "Eventos que publica".
- `packages/types/SPEC.md`: the 4 new types appended to the `src/catalogo.ts` row of the file-organization table, with a note crediting the same treatment `shared-types-package` gave the other 7 files in the foundation change.
- `catalogo.module.ts` exports audited: confirmed `exports: [CATALOG_QUERY_PORT]` exactly — the doc comment above it already asserted this, verification just confirmed the comment matches the code.
- Full verification, all green: `pnpm lint`/`typecheck`/`test` (235 unit + 54 e2e)/`build`/`format:check`, plus the opt-in integration suite (3 suites / 10 tests) run once more for extra confidence before closing out.
- The 6 design.md-named open items (RLS-bypass reads the projection, no self-catalog-listing endpoint, `ajustarPreciosPorCategoria` 204-no-count, cross-upload `(nombre,categoria)` collision, unmeasured timeout defaults, `aprobarEmpresa`'s missing state precondition) are all carried forward in this file's cumulative record — none silently dropped. They are follow-ups for a future change, not blockers for this one.
